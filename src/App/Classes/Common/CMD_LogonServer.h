...
typedef struct _stCmdGpLogonAccounts
{
    ...
    BYTE							cbValidateFlags;                //校验标识
    TCHAR							szPwd[LEN_MD5];					//登录密码
    TCHAR							szAccount[LEN_MAX_ACCOUNT];			//登录帐号
...
typedef struct _stCmdGpRegAccount
{
	...
	TCHAR							szSpreader[LEN_MAX_ACCOUNT];			//推荐帐号
...
typedef struct _stCmdGpLogonSucc
{
    ...
    TCHAR							szAccounts[LEN_MAX_ACCOUNT];		//登录帐号
    TCHAR							szNickName[LEN_MAX_ACCOUNT];		//用户昵称
...
#define SUB_GP_MODIFY_BANK_PWD			102							//修改密码
...
typedef struct _stCmdGpModifySignature
{
    ...
    TCHAR							szSignature[LEN_SIGNATURE];			//个性签名
...
typedef struct _stCmdGpUserGoldTransfer
{
    ...
    TCHAR							szAccounts[LEN_MAX_NICKNAME];			//目标用户
    TCHAR							szMachineID[LEN_MACHINE_ID];		//机器序列
...
typedef struct _stCmdGpUserInfoQueryReq
{
    ...
    TCHAR							szAccounts[LEN_MAX_ACCOUNT];			//目标用户
...
typedef struct _stCmdGpUserTransferUserInfo
{
    ...
    TCHAR							szAccounts[LEN_MAX_ACCOUNT];			//目标账户
...
typedef struct _stCmdMbLogonAccounts
{
    ...
    TCHAR							szLogonAccounts[LEN_MAX_ACCOUNT];		//登陆帐号
...
typedef struct _stCmdMbRegAccounts
{
    ...
    WORD							wFaceID;							//头像标识
    BYTE							cbGender;							//用户性别
    TCHAR							szAccounts[LEN_MAX_ACCOUNT];			//注册帐号
    TCHAR							szNickName[LEN_MAX_NICKNAME];			//用户昵称
...
typedef struct _stCmdMbLogonSucc
{
    ...
    DWORD							dwExp;								//经验数值
    DWORD							dwLoveLiness;						//用户魅力
    TCHAR							szAccounts[LEN_MAX_ACCOUNT];			//用户帐号
    TCHAR							szNickName[LEN_MAX_ACCOUNT];			//用户昵称
    TCHAR							szDynamicPwd[LEN_PWD];				//动态密码
