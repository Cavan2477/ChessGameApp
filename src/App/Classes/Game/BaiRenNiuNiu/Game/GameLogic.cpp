...
int CGameLogic::RetType(int itype)
{
	...
	{
	case 0:
		return BRNN_CT_SPECIAL_NIUNIU;
	case 1:
		return BRNN_CT_SPECIAL_NIU1;
		break;
	case 2:
		return BRNN_CT_SPECIAL_NIU2;
		break;
	case 3:
		return BRNN_CT_SPECIAL_NIU3;
		break;
	case 4:
		return BRNN_CT_SPECIAL_NIU4;
		break;
	case 5:
		return BRNN_CT_SPECIAL_NIU5;
		break;
	case 6:
		return BRNN_CT_SPECIAL_NIU6;
		break;
	case 7:
		return BRNN_CT_SPECIAL_NIU7;
		break;
	case 8:
		return BRNN_CT_SPECIAL_NIU8;
		break;
	case 9:
		return BRNN_CT_SPECIAL_NIU9;
		break;
	default :
		return BRNN_CT_POINT;
		break;
	}
...
BYTE CGameLogic::GetCardType(const BYTE cbCardData[], BYTE cbCardCount,BYTE *bcOutCadData )
{
	...
	if (5!=cbCardCount) return BRNN_CT_ERROR;
	...
	if(GetCardNewValue(cbCardDataSort[0])==GetCardNewValue(cbCardDataSort[cbCardCount-2]))
	{
		...
		return BRNN_CT_SPECIAL_BOMEBOME;
	}else
	{
	...
	if(blBig)
	{
		...
		if(iValueTen==0)
			return BRNN_CT_SPECIAL_NIUKING;
		else
		{
			if(iValueTen==1)
			{
				return BRNN_CT_SPECIAL_NIUYING;
			}
		}
	}
	...
	if( iGetTenCount>=3)
	{
		if(GetCardColor(cbCardDataSort[0])==0x04&&GetCardColor(cbCardDataSort[1])==0x04)
		{
			...
			return BRNN_CT_SPECIAL_NIUNIUDW;
		...
		if(GetCardColor(cbCardDataSort[0])==0x04)
		{
			...
			if(cbCardDataSort[0]==0x42)
				...
				return BRNN_CT_SPECIAL_NIUNIUXW;
		}else
		{
...
int CGameLogic::CompareCard(const BYTE cbFirstCardData[], BYTE cbFirstCardCount,const BYTE cbNextCardData[], BYTE cbNextCardCount,BYTE &Multiple)
{
    ...
    int ptest = cbFirstCardType;
    int ptest1 = cbNextCardType;
	//牌型比较
	if (cbFirstCardType != cbNextCardType) 
	{
		if (cbNextCardType > cbFirstCardType)
		{
			if(cbNextCardType >=BRNN_CT_SPECIAL_NIUNIU && cbNextCardType<=BRNN_CT_SPECIAL_BOMEBOME)	Multiple = 10;
			else if(cbNextCardType>=BRNN_CT_SPECIAL_NIU1 && cbNextCardType<=BRNN_CT_SPECIAL_NIU9)	Multiple = cbNextCardType-1;
			else if(cbNextCardType == BRNN_CT_POINT)	Multiple = 1;
			else Multiple = 1;
			return 1;
		...
		{
			if(cbFirstCardType >=BRNN_CT_SPECIAL_NIUNIU && cbFirstCardType<=BRNN_CT_SPECIAL_BOMEBOME)	Multiple = 10;
			else if(cbFirstCardType>=BRNN_CT_SPECIAL_NIU1 && cbFirstCardType<=BRNN_CT_SPECIAL_NIU9)	Multiple = cbFirstCardType-1;
			else if(cbFirstCardType == BRNN_CT_POINT)	Multiple = 1;
			else Multiple = 1;
	...
	//特殊牌型判断
	if (BRNN_CT_POINT!=cbFirstCardType && cbFirstCardType==cbNextCardType)
	{
		//排序扑克
