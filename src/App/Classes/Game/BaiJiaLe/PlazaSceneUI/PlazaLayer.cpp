...
#include "../CheckInSceneUI/CheckInLayer.h"*/
        ...
        switch (pWidget->getTag())
        {
            ...
            case TAG_TASK_BTN:
            {
                /*
                 ...
                 Director::getInstance()->replaceScene(reScene);
                 */
                INSTANCE(SceneMgr)->transitionScene(BJL_SCENE(TASK_SCENE),false);
            }
                break;
            case TAG_BANK_BTN:
            {
                ...
                if (INSTANCE(GameDataMgr)->getEnterNormalRoom()
                    ...
                    && INSTANCE(GameDataMgr)->getEnterRoom() < HallDataMgr::getInstance()->m_roomList.size())
                {
                    tagGameServer *pServer = HallDataMgr::getInstance()->m_roomList.at(INSTANCE(GameDataMgr)->getEnterRoom());
                    if (nullptr != pServer)
                    {
                ...
                INSTANCE(SceneMgr)->transitionScene(BJL_SCENE(BANK_SCENE),false);
            }
                break;
            case TAG_REWARD_BTN:
            {
            ...
            case TAG_SET_BTN:
            {
                if (nullptr == m_settingLayer)
                {
                    ...
                    m_root->addChild(m_settingLayer,PLAZA_CONTROL_ZORDER);
                }
                if (m_tableList)
                {
                    m_tableList->enableTableList(false);
            ...
            case TAG_LOCKDESK:
            {
                ...
                m_passwdLayer->refreshDefaultInput(HallDataMgr::getInstance()->m_Tablepass);
...
{
    m_tableMenuLayout = static_cast<Layout*>(m_root->getChildByName("table_menu"));
...
void PlazaLayer::initPasswdLayout()
{
    if (nullptr == m_passwdLayer)
    {
        ...
        m_root->addChild(m_passwdLayer,PLAZA_CONTROL_ZORDER);
    }
}
...
void PlazaLayer::showTableList(bool bShow)
{
    ...
    if (INSTANCE(GameDataMgr)->getEnterAntiCheat())
    {
        BJL_NAMESPACE_FUN(AntiCheatLayer) *pLayer = BJL_NAMESPACE_FUN(AntiCheatLayer)::createLayer(this);
...
{
    if (show && m_root->getChildByTag(TAG_LEVEL_DATA))
    {
        return;
    }
    if (!show)
    {
        m_root->removeChildByTag(TAG_LEVEL_DATA);
        return;
    }
    if (show)
    {
        ...
        currPerience->setTextColor(Color4B::BLACK);
        currPerience->setAnchorPoint(Vec2(0.f, 0.f));
        currPerience->setPosition(Vec2(fXpos, pbg->getContentSize().height-60));
        ...
        currlevel->setTextColor(Color4B::BLACK);
        currlevel->setAnchorPoint(Vec2(0.f, 0.f));
        currlevel->setPosition(Vec2(fXpos, pbg->getContentSize().height-120));
        pbg->addChild(currlevel);
        
        if (HallDataMgr::getInstance()->m_levelData.wCurrLevelID < GameMaxLevel)
        {
            auto upgrade = Text::create("*升级奖励 :", FontBold, 25);
            upgrade->setTextColor(Color4B::BLACK);
            upgrade->setAnchorPoint(Vec2(0.f, 0.f));
            upgrade->setPosition(Vec2(fXpos, pbg->getContentSize().height-160));
