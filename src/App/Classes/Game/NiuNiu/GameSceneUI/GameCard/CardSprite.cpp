...
CardSprite* CardSprite::createCard(const BYTE &cardData, const char* backfile)
{
    ...
    {
        sp->autorelease();
        BYTE value = INSTANCE(GameLogic)->GetCardValue(cardData);
        BYTE color = (GameLogic::getInstance()->GetCardColor(cardData) >> 4) + 1;
        //TODO color的byte值
...
{
    BYTE cardValue = INSTANCE(GameLogic)->GetCardValue(value);
    BYTE cardColor = (GameLogic::getInstance()->GetCardColor(value) >> 4) + 1;
