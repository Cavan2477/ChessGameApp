...
bool TableItemView::init()
{
    ...
    do
    {
        ...
        for (int i = 0; i < max; ++i)
        {
            ...
            m_spReady[i]->setPositionY(-10);
            
            sprintf(buf, "name%d",i + 1);
            tmp = m_root->getChildByName(buf);
            CC_ASSERT(tmp != nullptr);
            m_clipUserName[i] = ClipText::createClipText(Size(87, 23), "","Arial",14);
...
void TableItemView::refreshTableItem(const tagTableItem* tableItem)
{
    ...
    for (int i = 0; i < max; ++i)
    {
        ...
        if (tableItem->tableUsers[i].wUserId != INVALID_USERID)
        {
            ...
            //昵称
            UserData *pUser = HallDataMgr::getInstance()->m_UserList.at(tableItem->tableUsers[i].wUserId);
            if (nullptr != pUser)
            {
                //m_textUserName[i]->setString(pUser->m_nickname);
                ...
                if (nullptr == m_pChairs[i])
                {
                    if (pUser->m_date.dwUserID == HallDataMgr::getInstance()->m_dwUserID)
                    {
                        ...
                        {
                            m_pChairs[i] = HeaderRequest::createwithFaceID(
                    ...
                    Sprite *pSen = Sprite::create("plaza/desk_noBody.png");
