...
#ifndef __NN_GAME_SCENE_H__
#define __NN_GAME_SCENE_H__
...
class GameScene : public UIScene, public UserItemDelegate
{
public:
	GameScene();
	~GameScene();
	...
	virtual bool uiMsg(const int &main, const int &sub,void *pData, const int &nLen) override;	// ui线程消息

	CC_SYNTHESIZE(WORD, m_wGameTable, GameTable);
	CC_SYNTHESIZE(WORD, m_wGameChair, GameChair);
	CC_SYNTHESIZE(bool, m_bSitDown, SitDown);
    CC_SYNTHESIZE(bool, m_bClickExit, ClickExit);		//点击确认离开游戏
